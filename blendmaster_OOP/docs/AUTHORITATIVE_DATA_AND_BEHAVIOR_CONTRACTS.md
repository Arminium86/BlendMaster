...
- Calendar owns a separate crusher section for each selected physical tipping point, with per-period rate, direct-tip ratios, grade limits and one total Max Reclaim Rate. This replaces the former stockpile-by-route capacity editor. The solver enforces that reclaim cap across all stockpile sources feeding the point. Old route-capacity entries are removed when the revised setup is submitted; existing point targets migrate to Calendar.
