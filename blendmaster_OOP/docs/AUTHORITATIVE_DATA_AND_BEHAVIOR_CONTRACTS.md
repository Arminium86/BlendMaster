...
### Plan Mode and Calendar ownership correction (11 September 2026)

This user clarification supersedes the initial Tasks 28–29 placement and reclaim-capacity inputs above.

- Site Configuration owns Plan Mode. Single tipping point permits one OPF and one operating crusher; multiple tipping points → one OPF permits one OPF and multiple physical crushers; Combined OPF requires multiple OPFs and at least one selected operating crusher for each. Synthetic Total_Feed is not offered as a physical crusher. Multi-selections survive scenario/project restoration.
- The dedicated Setup > Multi-feed Setup page is enabled only for the two simultaneous modes. Decision Levers no longer contains these controls. Tipping-point, ROM-area, grade-stream, direct-tip and Rehandle Movement Rules choices use known selections. The rule column is named Stockpile subset. Tipping-point OPF is derived from Site Configuration.
- Calendar owns a separate crusher section for each selected physical tipping point, with per-period rate, direct-tip ratios, grade limits and one per-source Max Reclaim Rate. This replaces the former stockpile-by-route capacity editor. The solver applies that reclaim cap independently to each eligible inventory stockpile or active AMT chunk feeding the point; crusher rate caps combined feed. Old route-capacity entries are removed when the revised setup is submitted; existing point targets migrate to Calendar.
- Brand is read-only in these Calendar sections and comes from the active receiving Product Target. Product Targets uses a dropdown of selected OPFs, with multi-selection for shared builds. Automatic 2WP targets are fetched once per owning OPF in simultaneous mode, avoiding duplicated targets across its crushers.
- OPF reconciliation selects independently prepared Data Streams scenarios for each OPF at the same mine/start. It remains necessary to preserve OPF-specific factors; the page now explains this purpose and auto-selects a unique compatible scenario. It is available only in Combined OPF mode.
- 405 automated regression checks passed. Native UI validation covered mode cardinality, dropdowns, Calendar submission and invalid-entry handling, actual project save/load, and returning to single mode. Solver integration confirms a 250 t/h crusher with a 180 t/h reclaim cap feeds 180 t/h from stockpiles, sharing the original inventory ledger. Screenshots and the revised review guide are local ignored artifacts in `docs/screenshots/task29/` and `docs/TASK_28_29_PLAN_MODE_REVIEW.md`.

### Combined OPF reconciliation within one scenario (12 September 2026)

This confirmed clarification supersedes the separate reconciliation scenarios described above.

- Multi-feed Setup no longer has an OPF reconciliation tab or scenario selector. Legacy scenario selections are discarded. One Combined OPF scenario can prepare and run every selected OPF independently.
- Data Streams freezes the selected OPFs, fetches each OPF's standard factors and advanced history, and retains those inputs in the owning scenario/project. Each OPF uses its own history requirements and fallback warnings. An unavailable OPF receives the existing explicit default-factor fallback, never another OPF's factors. Cached evidence is invalidated by relevant mine, start, brands, inventory-build and history requirements.
- Each OPF's inventory and AMT grades are derived from that scenario's common physical sources, using its own reconciliation settings/history. Known product-slot mappings follow the receiving OPF: CC OPF02 uses inventory PROD3 and AMT/APS PROD2. Unrelated custom mappings remain explicit. APS remains pre-adjusted; historical factors are not reapplied to APS grade blocks.
...
- Opening transport Grade Control enrichment uses `RECORD_ACTIVE_FLAG = 'Y'` and the latest `RECORD_CREATED_DT` per full block name, consistent with nominal-block lookup. Inactive revisions cannot create false model conflicts. Identical latest rows collapse without duplicating movement tonnes; conflicting rows tied for latest remain an explicit ambiguity. Missing active models retain the existing explicit Inventory stream-mass fallback. Opening-history request version 3 invalidates caches prepared under the unfiltered model lookup.
