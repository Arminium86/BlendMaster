...
def saved_review(state):
    ...
    for opf in opfs(state):
        ...
        for name, kind, row, hex_id, build in selected_sources(state):
            ...
            managed = continuous_inventory(state, opf, name, row) if kind == 'inventory' else None
            if managed:
                audit = deepcopy(managed['reconciliation'])
                audit.update(prediction_scope='inventory', last_adjusted=managed.get('last_adjusted') or audit.get('last_adjusted'))
            else:
                details, stale = {}, False
                for brand in brands:
